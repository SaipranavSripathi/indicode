...
// IndiCode ships with Sarvam AI and Krutrim AI pre-wired as OpenAI-compatible providers so
// students don't have to hand-write opencode.json. Users still need to set the API key env vars
// (or run `/connect`) - see README for how to get a Sarvam AI / Krutrim AI key.
const INDICODE_DEFAULT_CONFIG = {
  ...
  provider: {
    ...
    "krutrim-ai": {
      ...
      options: {
        ...
        apiKey: "{env:KRUTRIM_API_KEY}",
      },
      models: {
        "krutrim-1": { name: "Krutrim-1" },
      },
    },
  },
}
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify(INDICODE_DEFAULT_CONFIG, null, 2))
